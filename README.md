...
**目前最新版本：[v1.2.0](docs/releases/v1.2.0.md)**
...
- `LaunchFlags`：每個帳號額外要帶的 **D2R 啟動旗標**；可以先留空，工具會自動 fallback 成 `0`，之後再回到主選單用 `f` 設定。目前工具只保留 `-ns`（關閉聲音）這一種 flag；若你想讓不同帳號用不同畫質，請改用主選單 `g` 的 `GraphicsProfile`。舊版若還留有 `-lq`，launcher 讀取時會自動清掉並回寫。參數用途可再查 [docs/D2R_PARAMS.md](docs/D2R_PARAMS.md)
- `ToolFlags`：每個帳號的 **工具內部設定**（bitmask）；可以留空，預設為 `0`。目前支援：`1` = 從 switcher 切換循環中排除此帳號。可在主選單 `s → [2]` 設定，不需手動填寫。
- `GraphicsProfile`：每個帳號要套用的 **具名畫質設定檔**；可以先留空。若留空，啟動該帳號時工具就**完全不會動** `%USERPROFILE%\Saved Games\Diablo II Resurrected\Settings.json`。建議用主選單 `g` 先把目前遊戲設定存成具名 profile，再指派給帳號。
- `DefaultRegion`：這個帳號預設要登入的區域 server；可以先留空。建議用主選單 `v` 指派。之後啟動指定帳號或 `a` 批次啟動時，進入 region 選單直接按 Enter 就會使用這裡的設定；若本次目標裡有人沒設定，工具會擋下並列出帳號，仍可改成手動選 `1` / `2` / `3`。
- `DefaultMod`：這個帳號預設要載入的 mod；可以先留空。建議用主選單 `m` 指派。若你想讓 Enter 預設走原版，可設成 `<vanilla>`（CLI 選單中等同於「不使用 mod」）。之後啟動指定帳號或 `a` 批次啟動時，進入 mod 選單直接按 Enter 就會使用這裡的設定；若本次目標裡有人尚未設定有效的預設 mod，工具會擋下並列出帳號。若某個已保存的 `DefaultMod` 指向當前 D2R 安裝下已不存在的 mod，launcher 會先自動清空該帳號的 `DefaultMod`，再把它視為未設定。
...
- `<數字>`：先進入 region 選單；畫面會先列出這次準備啟動的帳號，並顯示每個帳號目前預設準備使用的 region / mod / 畫質設定。若該帳號已設定 `DefaultRegion`，可直接按 Enter 使用預設值，也可手動選 `1` / `2` / `3` 覆蓋這次啟動。接著再進入 mod 選單；若該帳號已設定 `DefaultMod`，也可直接按 Enter 使用預設值，或手動選 `0` / 其他編號覆蓋這次啟動。最後才會啟動指定帳號；若該帳號已在執行中會直接阻止重複啟動
- `a`：工具會先預掃描目前已開啟帳號；如果還有待啟動帳號，才會再讓你選區域與 mod。這兩個選單都會先列出本次待啟動帳號，並顯示每個帳號預設準備使用的 region / mod / 畫質設定。區域選單可直接按 Enter 讓每個待啟動帳號吃自己的 `DefaultRegion`，mod 選單也可直接按 Enter 讓每個待啟動帳號吃自己的 `DefaultMod`；你也可以手動選 `1` / `2` / `3` 或 `0` / 其他 mod 編號，替本次批次全部帳號做單次覆蓋
- `0`：先選一次要套用的已安裝 mod（如果有），再進離線模式
- `d`：設定 `a` 批次啟動時的啟動間隔；可輸入 `30` 或 `30-60` 這種範圍，代表每次都在該區間內隨機等待，且下限固定不可低於 10 秒
- `f`：先顯示帳號列表與置中的兩行 flag 對照表，再設定或取消各帳號的額外啟動 flag；目前只保留「關閉聲音」這一種 flag。若你要做每帳號不同畫質，請改用 `g` 的畫質設定檔。進入第二層後，前兩個選項會依目前動作自動切成「設定」或「取消」版本，另可直接「設定 / 取消所有帳號所有 flag」，各旗標用途可查 [docs/D2R_PARAMS.md](docs/D2R_PARAMS.md)
- `g`：管理帳號畫質設定檔。你可以先在遊戲裡把畫質 / 視窗 / 解析度調好，再回 CLI 把目前的 `%USERPROFILE%\Saved Games\Diablo II Resurrected\Settings.json` 存成具名 profile，接著像 flag 一樣把它指派給帳號、清除帳號指派，或刪除已保存但不再需要的 profile；若帳號沒指派 profile，啟動時就不會去覆蓋 `Settings.json`
- `m`：管理帳號預設 mod。你可以像設定畫質檔那樣，依 mod 選帳號或依帳號選 mod，替每個帳號預先綁定某個已安裝 mod；若你想讓某個帳號 Enter 時預設走原版，也可以把它設成「不使用 mod」。之後啟動指定帳號或 `a` 批次啟動進入 mod 選單時，直接按 Enter 就會使用這裡的預設值。若本次目標裡有任何帳號尚未設定有效的預設 mod，工具會擋下並列出帳號；若某個已保存的預設 mod 已不存在，launcher 會先自動清空該帳號的 `DefaultMod`
- `v`：管理帳號預設登入區域。你可以像設定畫質檔那樣，依區域選帳號或依帳號選區域，替每個帳號預先綁定 `NA` / `EU` / `Asia`；之後啟動指定帳號或 `a` 批次啟動進入 region 選單時，直接按 Enter 就會使用這裡的預設值。若本次目標裡有任何帳號尚未設定預設區域，工具會擋下並列出帳號，避免偷偷 fallback 到別的區域
- `p`：開啟 Windows 檔案選擇視窗，設定 `D2R.exe` 路徑
- `s`：設定視窗切換快捷鍵／滑鼠側鍵／搖桿按鍵
- `l`：切換工具介面語言（繁體中文 / English）；選定後立即生效並存入 `config.json`
...
- 若你要讓不同帳號套不同畫質，請先進遊戲調整好設定，再回主選單 `g` 把**目前** `Settings.json` 儲存成具名畫質設定檔後再指派給帳號。最保守做法是先離開遊戲，再回 launcher 儲存，避免 D2R 尚未把最新設定寫回磁碟。
- 若你已替帳號設定 `DefaultMod`，啟動指定帳號或 `a` 批次啟動時，進入 mod 選單可直接按 Enter 使用預設值；若本次目標裡有任何帳號尚未設定有效的預設 mod，工具會明確擋下並列出帳號，你可以回主選單輸入 `m` 補設定，或當下手動選 `0` / 其他 mod 編號
- 若你已替帳號設定 `DefaultRegion`，啟動指定帳號或 `a` 批次啟動時，進入區域選單可直接按 Enter 使用預設值；若本次目標裡有任何帳號尚未設定，工具會明確擋下並列出帳號，你可以回主選單輸入 `v` 補設定，或當下手動選 `1` / `2` / `3`
- 若某個已保存的畫質設定檔仍被帳號使用，launcher 會先阻止你刪除它；請先清除或改派那些帳號，再刪除該 profile。
- 若某個帳號指派的畫質設定檔在啟動時已不存在，launcher 會跳過覆蓋 `Settings.json`，並自動清空該帳號的 `GraphicsProfile` 指派，避免之後每次啟動都卡在同一個壞設定。
- 若某個帳號保存的 `DefaultMod` 指向目前 D2R 安裝下已不存在的 mod，launcher 會在玩家按 Enter 想使用預設 mod 時自動清空該帳號的 `DefaultMod`，並把這個帳號視為「未設定」，避免默默用錯 mod 或偷偷退回原版。
- 盡量不要手動修改 `config.json`，避免不小心破壞 JSON 格式；大部分設定請優先透過工具內建選單調整
- 語言設定儲存在 `config.json` 的 `language` 欄位（`"zh-TW"` 或 `"en"`）；若欄位缺少，工具下次啟動時會再跑一次語言選擇流程
