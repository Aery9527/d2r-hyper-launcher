...
**目前最新版本：[v1.0.0](docs/releases/v1.0.0.md)**
...
- `f`：設定或取消各帳號的額外啟動 flag，可用「以 flag 選帳號」或「以帳號選 flag」兩種方式批次調整；各旗標用途可查 [docs/D2R_PARAMS.md](docs/D2R_PARAMS.md)
...
- `a` 批次啟動預設的 `launch_delay` 是 30 秒。這是實際測試後較安全的建議值，因為如果短時間內太頻繁重複登入／關閉，Battle.net 端有機率把連線擋住；具體會擋多久不一定，但原則上就是盡量避免短時間內連線太多次；如果你要調整，請在主選單輸入 `d`，並注意下限固定不可低於 10 秒
