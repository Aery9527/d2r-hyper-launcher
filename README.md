...
- `<數字>`：先進入 region 選單；若該帳號已設定 `DefaultRegion`，可直接按 Enter 使用預設值，也可手動選 `1` / `2` / `3` 覆蓋這次啟動。之後再選一次要套用的已安裝 mod，最後啟動指定帳號；若該帳號已在執行中會直接阻止重複啟動
- `a`：工具會先預掃描目前已開啟帳號；如果還有待啟動帳號，才會再讓你選區域與 mod。此時可直接按 Enter 讓每個待啟動帳號吃自己的 `DefaultRegion`，也可手動選 `1` / `2` / `3` 給本次全部帳號共用
