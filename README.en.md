...
- `<number>`: first open the region menu. The prompt shows the target account along with the default region / mod / graphics profile currently prepared for Enter-based launch. If the account already has `DefaultRegion`, you can press Enter to use it, or choose `1` / `2` / `3` to override this launch manually. Then the mod menu appears; if the account already has `DefaultMod`, you can press Enter to use it, or choose `0` / another mod number to override this launch manually. The specified account is then launched; if it is already running, re-launch is blocked
- `a`: the tool pre-scans which accounts are already running; if there are pending accounts, it then asks for region and mod. Both menus first list the pending accounts together with the default region / mod / graphics profile currently prepared for Enter-based launch. In the region menu you can press Enter so each pending account uses its own `DefaultRegion`, and in the mod menu you can press Enter so each pending account uses its own `DefaultMod`; you can also choose `1` / `2` / `3` or `0` / another mod number once to override the whole batch launch
