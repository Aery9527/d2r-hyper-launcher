...
- `accounts.csv` 行為是刻意設計：UTF-8 BOM、載入時 `LaunchFlags` / `ToolFlags` 清洗、`DefaultRegion` 要正規化成 `NA` / `EU` / `Asia`、目前內建 `LaunchFlags` 只保留 `-ns`（每帳號畫質改用 `GraphicsProfile`）、密碼以 `ENC:` + DPAPI 加密（`internal/multiboxing/account/account.go`）。
