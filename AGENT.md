...
- menu option 需要整齊對齊時，優先使用 `newMenuOptions()` 收集後再 `render()`；主選單若最後要附上固定 `q` 離開選項，優先使用 `mainMenuOptions(func(*cliMenuOptions))`；子選單若最後要附上固定 `b / h / q` 導航，優先使用 `subMenuOptions(func(*cliMenuOptions))`，由 UI layer 統一補上空行與共通選項
